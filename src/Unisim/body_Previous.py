...
class Body_RoundEarth(Body):
    """
    TODO: It should refer to the Orbiting Body, so it will update the corresponding parameters
    """
    def __init__(self, t0, x0, method='RK45', options=None):
    ...
    def altitude(self):
        """
        TODO: Get current orbiting body radius
        """
        return np.linalg.norm(self._state_vector[0:3]) - EARTH_MEAN_RADIUS

    def fun(self, t, x):
        """
        TODO: When vehicle hit ground, put it to "sleep"
        ...
        if np.linalg.norm(pos) < EARTH_MEAN_RADIUS:
            print("==================")
            print("Vehicle Hit Ground")
            print("==================")
            #exit(0)
